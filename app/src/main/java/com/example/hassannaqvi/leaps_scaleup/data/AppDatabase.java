...
@Database(entities = {Forms.class, Forms_04_05.class, Clusters.class, Users.class, Participant.class, Forms_GPS.class, FO.class}, version = AppDatabase.Sub_DBConnection.DATABASE_VERSION, exportSchema = false)
...
//            database.execSQL("ALTER TABLE forms " + " ADD COLUMN last_update TEXT");
    ...
    // Alter table for Database Update
    private static final Migration MIGRATION_v2_v3 = new Migration(2, 3) {
        @Override
        public void migrate(SupportSQLiteDatabase database) {
            database.execSQL("CREATE TABLE " + Sub_DBConnection.TABLE_FO + "("
                    + FO.FOTable._ID + " INTEGER,"
                    + FO.FOTable.COLUMN_F_ID + " TEXT,"
                    + FO.FOTable.COLUMN_F_NAME + " TEXT, PRIMARY KEY(" + FO.FOTable._ID + ") );");
        }
    };
